...
namespace YARG.Core.Song
{
    public sealed partial class AvailableParts
    {
        ...
        public void ParseMidi(byte[] file, DrumPreparseHandler drums)
        {
            ...
            {
                if (midiFile.TrackNumber == 1 || track.Type != MidiEventType.Text_TrackName)
                    continue;

                string name = Encoding.ASCII.GetString(track.ExtractTextOrSysEx());
                if (YARGMidiTrack.TRACKNAMES.TryGetValue(name, out var type) && type != MidiTrackType.Events && type != MidiTrackType.Beat)
                {
