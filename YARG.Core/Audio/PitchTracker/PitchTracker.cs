...
namespace YARG.Core.Audio
{
    ...
    public class PitchTracker
    {
        ...
        public PitchTracker(float detectLevelThreshold = 0.01f, float sampleRate = 44100f)
        {
            ...
            _samplesPerPitchBlock = (int) Math.Round(sampleRate / PITCH_RECORDS_PER_SECOND);
        ...
        public float? ProcessBuffer(ReadOnlySpan<float> input)
        {
            ...
            {
                var frameCount = Math.Min(input.Length - samplesProcessed, _pitchBufSize + _detectOverlapSamples);
