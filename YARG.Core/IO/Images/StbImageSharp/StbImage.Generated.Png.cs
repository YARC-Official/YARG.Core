...
namespace StbImageSharp
{
	unsafe partial class StbImage
	{
		...
		public static readonly byte[] first_row_filter =
			{ STBI__F_NONE, STBI__F_SUB, STBI__F_NONE, STBI__F_AVG_FIRST, STBI__F_SUB };

		public static readonly byte[] stbi__check_png_header_png_sig = { 137, 80, 78, 71, 13, 10, 26, 10 };
		public static readonly byte[] stbi__depth_scale_table = { 0, 0xff, 0x55, 0, 0x11, 0, 0, 0, 0x01 };

		public static bool stbi__png_test(stbi__context* s)
		{
		...
		public static bool stbi__png_load(stbi__context* s, int* x, int* y, int* comp, stbi__result_info* ri, out FixedArray<byte> result)
		{
            var p = new stbi__png
            {
                s = s
		...
		public static bool stbi__check_png_header(stbi__context* s)
		{
			var i = 0;
			for (i = 0; i < 8; ++i)
		...
		public static bool stbi__create_png_image_raw(stbi__png* a, byte* raw, uint raw_len, int out_n, uint x, uint y,
			int depth, int color)
		{
		...
		public static bool stbi__create_png_image(stbi__png* a, byte* image_data, uint image_data_len, int out_n,
		...
		public static void stbi__compute_transparency(stbi__png* z, byte* tc, int out_n)
		{
			var s = z->s;
			uint i = 0;
		...
		public static void stbi__compute_transparency16(stbi__png* z, ushort* tc, int out_n)
		{
			var s = z->s;
			uint i = 0;
			var pixel_count = s->img_x * s->img_y;
		...
		public static bool stbi__expand_png_palette(stbi__png* a, byte* palette, int len, int pal_img_n)
		{
			uint i = 0;
		...
		public static bool stbi__parse_png_file(stbi__png* z, int scan)
		{
		...
		public static bool stbi__do_png(stbi__png* p, int* x, int* y, int* n, stbi__result_info* ri, out FixedArray<byte> result)
		{
		...
		{
			if (!stbi__parse_png_file(p, STBI__SCAN_HEADER))
			{
				stbi__rewind(p->s);
				return 0;
			}
