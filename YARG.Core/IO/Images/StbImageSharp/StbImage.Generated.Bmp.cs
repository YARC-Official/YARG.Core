...
namespace StbImageSharp
{
    ...
    {
        private static bool stbi__bmp_test(stbi__context* s)
        {
        ...
        private static bool stbi__bmp_load(stbi__context* s, int* x, int* y, int* comp, stbi__result_info* ri, out FixedArray<byte> result)
        {
            uint mr = 0;
            uint mg = 0;
            uint mb = 0;
        ...
        private static bool stbi__bmp_test_raw(stbi__context* s)
        {
            if (stbi__get8(s) != 66)
                return false;
            if (stbi__get8(s) != 77)
        ...
        private static int stbi__bmp_set_mask_defaults(stbi__bmp_data* info, int compress)
        {
            if (compress == 3)
                return 1;
            if (compress == 0)
        ...
        private static bool stbi__bmp_parse_header(stbi__context* s, stbi__bmp_data* info)
        {
            var hsz = 0;
