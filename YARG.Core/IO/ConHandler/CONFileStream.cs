...
namespace YARG.Core.IO
{
    public sealed class CONFileStream : Stream
    {
        ...
        private readonly NativeArray<long> blockLocations;
        ...
        public CONFileStream(FileStream filestream, bool isContinguous, int fileSize, int firstBlock, int shift)
        {
            ...
            if (isContinguous)
            {
                dataBuffer = NativeArray<byte>.Alloc(BYTES_PER_SECTION);
                ...
                blockLocations = NativeArray<long>.Alloc(numBlocks);
            ...
            {
                dataBuffer = NativeArray<byte>.Alloc(BYTES_PER_BLOCK);
